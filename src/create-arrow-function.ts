...
export { createArrowFunction };
export { Match, isMatch, CreateArrowFunction };

function createArrowFunction(code: Code, selection: Selection): Modification {
  ...
  return new CreateArrowFunction(match, code);
}
...
class CreateArrowFunction implements Modification {
  constructor(protected match: Match, protected code: Code) {}

  execute(update: Update) {
    update({
      code: `\nconst ${this.name} = ${this.modifier}(${this.args}) => {\n  ${this.body}\n}${this.after}`,
