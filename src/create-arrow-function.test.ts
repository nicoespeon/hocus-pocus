...
import { createShouldUpdateCodeFor, createShouldNotUpdateCodeFor } from './test-helpers';

const shouldUpdateCodeFor = createShouldUpdateCodeFor(createArrowFunction);
const shouldNotUpdateCodeFor = createShouldNotUpdateCodeFor(createArrowFunction);

describe('Create arrow function declaration from a call expression', () => {
...
const readCode = () => {
  ...
  it('assignment called with await', () => {
    shouldUpdateCodeFor({
      code: 'async function doSomethingAsync() { const code = await readCode(); }',
...
function write(code) {}`,
  ...
  it('with params', () => {
    shouldUpdateCodeFor({
      ...
      expectedSnippet: {
        code: `
const readCode = (\${1:selection}, \${2:param2}, \${3:param3}) => {
  \${0:// Implement}
}`,
        name: 'Create arrow function "readCode"',
      },
